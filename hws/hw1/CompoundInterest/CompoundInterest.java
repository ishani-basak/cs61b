 ...
 *  computations.
 *  @author P. N. Hilfinger, Zoe Plaxco, Ishani Basak
 */
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate, int targetYear) {
        ...
        double percentage = (100 + rate) / 100;
    ...
    static double futureValueReal(double presentValue, double rate, int targetYear,
        ...
        double inflationPercentage = (100 - inflationRate) / 100;
        for (int i = 0; i < numYears(targetYear); i++) {
            nominalValue *= inflationPercentage;
        }
        return nominalValue;
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        ...
        double percentage = (100 + rate) / 100;
        for (int i = 0; i < numYears(targetYear); i++) {
            savings += (perYear * percentage);
            percentage *= (100 + rate) / 100;
        }
        return savings;
    }
    ...
    static void printDollarFV(int targetYear, double returnRate,
                              double inflationRate) {
        double nominalDollarValue = futureValue(
                1, returnRate, targetYear); // replace 0 with your code
        double realDollarValue = futureValueReal(
                1, returnRate, targetYear, inflationRate);    // replace 0 with your code
    ...
    static void printSavingsFV(int targetYear, double returnRate, double inflationRate,
                               double perYear) {
        double nominalSavings = totalSavings(
                perYear, targetYear, returnRate); // replace 0 with your code
        double realSavings = totalSavingsReal(
                perYear, targetYear, returnRate, inflationRate);    // replace 0 with your code
