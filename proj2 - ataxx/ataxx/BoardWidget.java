...
class BoardWidget extends Pad {
     ...
     * Stroke for blocks.
     */
    private static final BasicStroke BLOCK_STROKE = new BasicStroke(1.0f);
    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        for (char c = 'a'; c <= 'g'; c++) {
            for (char r = '1'; r <= '7'; r++) {
                PieceColor p = _model.get(c, r);
                int centerX = ((c - 'a') * SQDIM) + (SQDIM / 2);
                int centerY = ((r - '1') * SQDIM) + (SQDIM / 2);
                if (p.equals(BLOCKED)) {
                    drawBlock(g, centerX, centerY);
                } else if (p.equals(RED)) {
                    g.setColor(RED_COLOR);
                    g.fillOval(centerX - PIECE_RADIUS,
                            _dim - centerY - PIECE_RADIUS,
    ...
    void drawBlock(Graphics2D g, int cx, int cy) {
        ...
        g.fillRect(cx - (BLOCK_WIDTH / 2), cy - (BLOCK_WIDTH / 2),
    ...
    private void handleClick(String unused, MouseEvent where) {
        ...
        if (where.getButton() == MouseEvent.BUTTON1) {
            ...
            if (mouseCol >= 'a' && mouseCol <= 'g'
                ...
                if (_blockMode) {
                    _commandQueue.offer(String.format("block %c%c",
                            mouseCol, mouseRow));
                } else {
                    if (_selectedCol != 0) {
