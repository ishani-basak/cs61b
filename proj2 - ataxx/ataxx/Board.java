...
class Board {
    ...
    void clear() {
        ...
        for (int i = 0; i < _board.length; i++) {
            if (i % EXTENDED_SIDE >= 2 && i % EXTENDED_SIDE <= 8
                    && i >= EXTENDED_SIDE * 2 + 2
                    && i < EXTENDED_SIDE * EXTENDED_SIDE
                    - (EXTENDED_SIDE * 2 + 2)) {
                _board[i] = EMPTY;
            } else {
                _board[i] = BLOCKED;
    ...
    boolean legalMove(Move move) {
        if (move == null) {
            ...
            System.out.println(!canMove(whoseMove()));
            return !canMove(whoseMove());
        } else if (move.col0() < 'a' || move.col0() > 'g'
                || move.row0() < '1' || move.row0() > '7'
                || move.col1() < 'a' || move.col1() > 'g'
                || move.row1() < '1' || move.row1() > '7') {
            return false;
        } else if (_board[move.fromIndex()] != whoseMove()) {
            return false;
        } else if (move.isExtend() || move.isJump()) {
            return _board[move.toIndex()] == PieceColor.EMPTY;
        } else {
            return true;
        }
    }
    ...
    int numJumps() {
        int jumps = 0;
        for (int m = 0; m < _allMoves.size(); m++) {
            if (_allMoves.get(m).isJump()) {
                jumps++;
            } else {
                jumps = 0;
            }
    ...
    void makeMove(Move move) {
        ...
        if (move.isExtend()) {
            set(move.toIndex(), _board[move.fromIndex()]);
        ...
        for (int r = -1; r <= 1; r++) {
            for (int c = -1; c <= 1; c++) {
                int checkSquare = neighbor(move.toIndex(), c, r);
                if (_board[checkSquare] == opponent) {
                    _undoSquares.add(checkSquare);
                    _undoPieces.add(_board[checkSquare]);
                    set(checkSquare, _board[move.toIndex()]);
                    incrPieces(whoseMove(), 1);
                    incrPieces(whoseMove().opposite(), -1);
                }
            }
        }
        if (numPieces(whoseMove().opposite()) == 0) {
            _winner = whoseMove();
        } else if (numJumps() >= JUMP_LIMIT
                || (!canMove(RED) && !canMove(BLUE))) {
            if (numPieces(whoseMove()) > numPieces(whoseMove().opposite())) {
                _winner = whoseMove();
            } else if (numPieces(whoseMove())
                    < numPieces(whoseMove().opposite())) {
    ...
    boolean legalBlock(char c, char r) {
        if (_allMoves.isEmpty() && get(c, r) == EMPTY) {
            return true;
        }
        return false;
    }
    ...
    void setBlock(char c, char r) {
        ...
        ArrayList<Integer> blockLocs = new ArrayList<Integer>();
        blockLocs.add((r - '1' + 2) * EXTENDED_SIDE + (c - 'a' + 2));
        blockLocs.add(('7' - r + 2) * EXTENDED_SIDE + (c - 'a' + 2));
        blockLocs.add((r - '1' + 2) * EXTENDED_SIDE + ('g' - c + 2));
        blockLocs.add(('7' - r + 2) * EXTENDED_SIDE + ('g' - c + 2));
        for (int i : blockLocs) {
            if (get(i) == EMPTY) {
                set(i, BLOCKED);
