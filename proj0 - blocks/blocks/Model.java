...
class Model {
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        int overlap = (_height * nrows) + (_width * ncols);
        int noOverlap = overlap - (nrows * ncols);
        int increase = noOverlap + (_streakLength * overlap);
        return increase;
    }
