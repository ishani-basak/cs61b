/**
 * @author Vivant Sakore on 1/29/2020; updated by Linda Deng (1/26/2022), Ishani Basak
 */
...
public class BuggyIntDList extends IntDList {
    ...
    private DNode sortedMerge(DNode d1, DNode d2) {
        if (d1._val <= d2._val) {
            d1._next = sortedMerge(d1, d2._next);   /*FIXME*/
            d1._next._prev = d1;
            d1._prev = null;
            return d1;
        } else {
            d2._next = sortedMerge(d1._next, d2);   /*FIXME*/
            d2._next._prev = d2;
            d2._prev = null;
            return d2;
        }
