...
public class ArrayHeap<T> {
    ...
    private void bubbleUp(int index) {
        ...
        while (getNode(parentIndex) != null && getNode(index).priority() < getNode(parentIndex).priority()) {
    ...
    private void bubbleDown(int index) {
        // TODO
        int childIndex = minPriorityIndex(getLeftOf(index), getRightOf(index));
        while (childIndex <= size() && getNode(index).priority() >= getNode(childIndex).priority()) {
            swap(index, childIndex);
            index = childIndex;
            childIndex = minPriorityIndex(getLeftOf(childIndex), getRightOf(childIndex));
        }
    }
    ...
    public void changePriority(T item, double priority) {
        ...
        for (int i = 1; i <= size(); i++) {
            ...
            if (curr.item().equals(item)) {
                ...
                if (curr.priority() < getNode(getParentOf(i)).priority()) {
                    bubbleUp(i);
                } else {
                    bubbleDown(i);
                }
